...
**Hiểu trước khi nói.** Nếu model chỉ dự đoán chữ, tại sao Claude Code hay Cursor agent sửa được bug thật? Vì tool bọc model trong một vòng lặp gọi là ReAct (Reason + Act, từ paper của Yao và cộng sự năm 2022, nhóm Princeton và Google Brain). Paper này ghép hai trường phái trước đó vẫn tách rời. Trường phái một là chain-of-thought: bắt model suy luận từng bước trước khi trả lời; suy luận có tốt hơn, nhưng toàn bộ vẫn diễn ra "trong đầu" model, không chạm thế giới thật, nên sai từ giữa chừng là sai đến cùng mà không có gì kéo lại. Trường phái hai là action-only: model gọi hành động liên tiếp mà không viết suy nghĩ, nên không lập được kế hoạch và không tự đổi hướng được. ReAct xen kẽ hai thứ — một dòng suy nghĩ, một hành động, một quan sát, lặp lại — và điều paper đo được là bịa giảm hẳn so với chain-of-thought thuần, vì mỗi vòng model bị "tiếp đất" bằng một quan sát thật.

Cơ chế cụ thể trong một coding agent: system prompt khai báo danh sách tool (đọc file, tìm kiếm, sửa file, chạy lệnh). Mỗi lượt, model sinh ra một trong hai thứ: một đoạn suy nghĩ (Reason), hoặc một tool call có cấu trúc — tên tool cộng tham số (Act). Harness, tức phần vỏ bọc ngoài model, thực thi tool call đó thật trên máy, lấy output nguyên văn nối vào cuối context (Observe), rồi gọi model thêm một lần với toàn bộ context mới. Lặp đến khi model không gọi tool nữa hoặc chạm ngân sách bước (trên slide ghi step budget 30 — có ngân sách vì model có thể loay hoay vô hạn). Nghĩa là "agent" thực chất chỉ là model + một vòng while + một bộ tool; giữa hai lượt, model không nhớ gì ngoài context. Và vì mỗi vòng là một lần gọi model với toàn bộ context tích luỹ, phiên càng dài thì mỗi vòng càng đắt và càng dễ rot — đây là chỗ slide 6 quay lại.

Mỗi nút có một kiểu hỏng riêng, và đây là gốc của ba hệ quả SOP: Reason hỏng bằng một kế hoạch nghe trôi chảy nhưng sai (suy nghĩ vẫn chỉ là văn bản dự đoán); Act hỏng bằng gọi sai tool hoặc tool có quyền quá rộng; Observe hỏng khi không có gì đáng quan sát (repo không có test thì quan sát duy nhất là "file đã ghi") hoặc quan sát đúng nhưng kết luận sai.

Một trace mẫu đáng thuộc, vì nó biến ba nút thành hình ảnh cụ thể: test fail ở `billing.test.js` → Reason "cần đọc `billing.js`" → Act đọc file → Observe nội dung → Reason "lỗi làm tròn ở hàm total" → Act sửa hai dòng → Act chạy `node --test` → Observe "một test vẫn đỏ" → sửa tiếp → Observe "all pass" → dừng. Khoảng bốn năm vòng lặp, bấy nhiêu lần gọi model.

**Nói đơn giản.** Tool như Claude Code không "biết" gì hơn model, nhưng nó cho model một vòng lặp: nghĩ, làm một việc thật (đọc file, chạy test), nhìn kết quả thật, rồi nghĩ tiếp. Kết quả thật là thứ duy nhất nó tin được. Không có test để chạy thì nó chỉ có thể nói "xong rồi". Agent chỉ là model cộng một vòng while cộng một bộ tool — không có phép màu.
...
Cho ai thích gốc gác: ReAct đến từ một bài báo năm 2022 của nhóm Princeton và Google. Trước đó có hai trường phái tách rời nhau. Một bên là chain-of-thought: bắt model suy luận từng bước trước khi trả lời. Suy luận có tốt hơn thật, nhưng toàn bộ vẫn diễn ra trong đầu model, không chạm thế giới bên ngoài, nên nếu nó sai từ bước hai thì các bước sau sai theo rất mạch lạc, và không có gì kéo nó lại. Bên kia là cho model gọi hành động liên tiếp mà không viết suy nghĩ: làm được việc nhưng không có kế hoạch, lạc đường không tự biết. ReAct ghép hai thứ lại: nghĩ một bước, làm một bước, nhìn kết quả, rồi nghĩ tiếp. Và điều bài báo đo được: bịa giảm hẳn, vì cứ mỗi vòng model lại bị kéo về mặt đất bằng một quan sát thật.

Ba nút trên hình.

Nút một, Reason: model đọc mục tiêu và toàn bộ context, rồi viết ra một suy nghĩ về việc cần làm tiếp. Ví dụ: "test đang fail ở billing test, mình cần đọc billing.js trước". Chú ý: đây vẫn là văn bản, vẫn là đoán chữ, chỉ là đoán về hành động. Model không "quyết định" theo nghĩa con người; nó sinh ra câu mô tả hành động có xác suất cao nhất.

Nút hai, Act: model chọn một tool và gọi nó với tham số cụ thể. Đọc file này. Tìm chuỗi kia trong repo. Chạy node test. Sửa đoạn code từ dòng này đến dòng kia. Tool là thứ có thật, deterministic, chạy bên ngoài model, trên máy của bạn.

Nút ba, Observe: kết quả thật của tool được nạp ngược vào context. Nội dung file. Output của test, kể cả dòng đỏ. Lỗi compile. Đây là điểm duy nhất trong cả vòng lặp mà sự thật từ bên ngoài đi vào. Nhìn cái cổng nhỏ bên trái với dòng "tool output, the only way in". Tôi cố ý vẽ chỉ một cổng. Mọi thứ khác trong context — prompt của bạn, suy nghĩ của nó — đều là thứ ai đó viết ra, không phải thứ đã được kiểm.

Và tôi muốn giải mã luôn cho nó bớt vẻ phép màu. Đằng sau hình tròn này là một cấu trúc rất tầm thường: agent bằng model, cộng một vòng while, cộng một bộ tool. Mỗi lượt model sinh hoặc suy nghĩ, hoặc một tool call; phần vỏ ngoài — người ta gọi là harness — chạy tool thật, dán output nguyên văn vào cuối context, rồi gọi model thêm một lần với toàn bộ context mới. Chữ "until done" ở tâm nghĩa là: đến khi model không gọi tool nữa, hoặc chạm ngân sách ba mươi bước. Có ngân sách vì model có thể loay hoay vô hạn. Và để ý: mỗi vòng là một lần gọi model với toàn bộ context tích luỹ, nên phiên càng dài, mỗi vòng càng đắt và càng dễ rot. Slide sáu quay lại ngay tại đây.

Tôi kể một trace thật để ba nút thành hình ảnh cụ thể, cỡ ba mươi giây. Tôi giao: "test đang fail, sửa đi". Vòng một: nó nghĩ "cần đọc billing.js", đọc file, nhận về nội dung. Vòng hai: nó nghĩ "lỗi làm tròn ở hàm total", sửa hai dòng, nhận về "file đã ghi". Vòng ba: nó chạy node test, nhận về một test vẫn đỏ — đây là khoảnh khắc quan trọng nhất của cả phiên, sự thật vừa phủ định nó. Vòng bốn: nó sửa tiếp, chạy lại, tất cả xanh, dừng. Bốn vòng, bốn lần gọi model. Điểm tôi muốn mọi người thấy: agent không hề "biết" nó sửa đúng. Nó thấy dòng chữ all tests pass trong output, và dòng chữ đó là thứ duy nhất neo nó vào sự thật.

Chat thường thì ngược lại: một lần, prompt vào, câu trả lời ra, không có observe. Đó là toàn bộ khác biệt giữa hai loại công cụ, và cũng là lý do chúng hỏng khác nhau: chat hỏng bằng cách bịa API vì không có gì phủ định; agent hỏng bằng cách quan sát đúng nhưng kết luận sai, hoặc không có gì đáng để quan sát.

Ba hệ quả tôi muốn mọi người mang về, mỗi hệ quả ứng với một nút. Một, từ nút Observe: agent chỉ đáng tin bằng những gì tool cho nó thấy. Nếu repo không có test, bước observe chỉ có "file đã ghi", và agent kết luận xong vì không có gì nói ngược lại. Cho nó test, build, lint — ba chip xanh đi vào cổng — là thêm sự thật vào vòng lặp. Đây chính là lý do của câu "give the AI a check it can run" ở slide mười hai. Hai, từ nút Reason: đó là chỗ nó tự tin sai, nên khi review, mở log tool call ra đọc — nó đã đọc file nào, chạy lệnh gì, nhận về gì — quan trọng hơn đọc lời giải thích trôi chảy của nó. Ba, từ nút Act: đó là chỗ phải giới hạn quyền. Đọc file thì thoải mái. Chạy test thì được. Push, xoá, cài package thì phải qua người. Lát nữa Nhân sẽ kể chuyện một agent có quyền ghi sai đã làm gì.

Ai ở đây đang dùng agent mode hằng ngày? Lần gần nhất nó báo "done" mà bạn kiểm lại thấy chưa done, là hỏng ở nút nào — không có gì để quan sát, hay quan sát rồi kết luận sai?
...
Nếu ai thích một khung có tên để nhớ, có CREATE: Character là vai, Request là yêu cầu, Examples là ví dụ, Adjustments là ràng buộc, Type of output là định dạng, và Extras. Nhưng nói ngay để không ai học thuộc nhầm trọng tâm: khung chỉ là checklist nhắc đừng quên context. Thứ quyết định chất lượng câu trả lời vẫn là context, không phải khung.
...
Hai chi tiết hay bị hỏi ở đây. Một: EXPLAIN và EXPLAIN ANALYZE khác gì nhau? EXPLAIN cho kế hoạch dự kiến với chi phí ước lượng; EXPLAIN ANALYZE chạy thật câu query và cho thời gian, số dòng thực tế từng bước. Đưa cho AI cái thứ hai, vì ước lượng sai chính là nguyên nhân phổ biến của plan tệ. Nhưng nhớ: vì nó chạy thật, SELECT thì ổn, còn UPDATE hay DELETE phải bọc trong transaction rồi rollback. Hai: kèm cả version Postgres vào prompt, vì planner và tính năng index khác nhau giữa các version — cùng một câu SQL, hai version có thể ra hai plan khác nhau.
...
Ví dụ cho kỹ thuật cuối, vì nó trừu tượng nhất. Hình dung context window là một cái bàn làm việc nhỏ, còn repo là cả kệ sách sau lưng. Làm bài nghiên cứu, không ai bê cả kệ sách đổ lên bàn: chất ba chồng sách lên là hết chỗ viết, và lúc cần một trang thì lại không tìm thấy nó nằm đâu. Người làm việc gọn để bàn trống, viết tới chương nào thì xoay lại rút đúng cuốn đó, mở đúng trang. Google Maps cũng hoạt động đúng kiểu này: app không tải bản đồ cả thế giới về máy; nó tải đúng vài ô quanh chỗ bạn đứng, đi tới đâu tải tới đó. Just-in-time là vậy: thứ gì cần mới lên bàn.

Dịch sang agent, khác biệt nằm trong hai câu prompt. Cách cũ: dán nguyên UserService, dán schema, dán thêm năm file "cho đủ context", rồi mới hỏi — bàn đầy trước khi bắt đầu. Cách just-in-time: "Login đang trả 500, error nguyên văn đây, code trong thư mục src, tự mở file nào cần." Agent tự grep, tự mở đúng hai file, đúng lúc bước suy luận cần đến — chính là vòng lặp ReAct ở slide 7 đang rút sách khỏi kệ thay bạn. Thói quen rút ra: dán đường dẫn, đừng dán nội dung. Với doc thư viện cũng vậy: để nó lấy đúng trang, đúng version, khi chạm tới API đó.
...
Chuyện thật để mọi người hình dung vì sao lại thế. Tôi nhờ AI sửa một test đang fail. Nó sửa cái test thay vì sửa code. Tôi nhắc: sửa code, đừng sửa test. Nó xin lỗi rất lịch sự, rồi lại sửa test lần hai, theo một cách khác. Vì trong lịch sử phiên, nó đã tự neo vào cách hiểu "test sai", và mọi câu trả lời sau đều bị lịch sử kéo về cái neo đó — nhắc bao nhiêu lần cũng vậy, cái neo vẫn nằm trong context. Bấm clear, prompt mới dán error nguyên văn cộng file code, nó sửa đúng code ngay lần đầu. Không phải model khôn lên; chỉ là cái neo đã biến mất.
...
Một câu hỏi hay gặp: cụ thể với một bug thì nạp gì? Nạp vào: test đang fail, error nguyên văn, file đang sửa, file convention, và một ví dụ lân cận của pattern đang dùng. Bỏ ra: cả repo, các lần thử thất bại, file không liên quan "để có thêm context", secret và dữ liệu khách hàng.
...
**Hiểu trước khi nói.** Với task lớn, Anthropic đề xuất bốn bước: Explore (đọc, hiểu, không sửa; nhiều tool có plan mode), Plan (ra spec.md, bạn duyệt), Code (bước nhỏ, mỗi bước một test), Commit (checkpoint thường xuyên để quay lại được). Harper Reed có workflow nổi tiếng: prompt "hỏi tôi từng câu một để cùng xây spec chi tiết", ra spec.md, rồi prompt_plan.md và todo.md, rồi thực thi từng bước. Điều khiến agent chạy được một mình là có "check tự chạy": test, build, lint. Không có test, agent chỉ có thể báo xong. Trong Claude Code, bốn ga là bốn cơ chế có thật: plan mode (Shift+Tab, mọi tool ghi bị khoá) cho Explore; màn duyệt kế hoạch — agent trình plan, bạn bấm approve rồi nó mới được sửa file — cho Plan; hook Stop cho Code: một đoạn config chạy test đúng lúc agent định kết thúc lượt, test đỏ thì hook trả exit code 2 và agent buộc phải sửa tiếp, không được báo done; checkpoint git cho Commit. Ví dụ config nằm trong CLAUDE-MAPPING.md của repo. Một lưu ý riêng: training cutoff — thư viện có breaking change sau thời điểm train thì phải dán doc mới vào prompt, không thì model dùng API cũ một cách tự tin.
...
Bốn ga này không phải quy trình trên giấy. Nếu bạn dùng Claude Code, mỗi ga là một cơ chế có thật, tôi đi nhanh một vòng. Explore là plan mode vừa nói: Shift Tab, mọi tool ghi bị khoá. Plan là màn duyệt kế hoạch của chính tool: nó trình kế hoạch, bạn bấm approve rồi nó mới được sửa file — con dấu APPROVED trên slide chính là cái nút đó, không phải ẩn dụ. Code là chỗ đắt nhất: hook Stop, một đoạn config chạy test đúng lúc agent định kết thúc lượt; test đỏ thì hook chặn lại kèm thông báo, agent không được phép nói done và phải sửa tiếp. Để ý điều vừa xảy ra: câu "a check it can run" từ lời khuyên đã trở thành cơ chế — máy giữ kỷ luật thay cho người ngồi canh. Commit thì là checkpoint git bình thường. Cursor hay tool khác đều có các cơ chế tương đương, khác mỗi tên gọi; bản map chi tiết từng slide sang config có trong file CLAUDE-MAPPING.md của repo này, mọi người đọc sau buổi.
...
Để hình dung cuộc phỏng vấn đó trông ra sao, lấy một task thật: thêm pagination cho toàn bộ list endpoint. Ba câu đầu tiên model sẽ hỏi: cursor hay offset? Page size mặc định và tối đa là bao nhiêu? Response envelope đổi thế nào để client cũ không vỡ? Để ý: toàn là quyết định thiết kế, và nếu để nó tự đoán thì bạn sẽ sửa chúng sau khi ba trăm dòng code đã viết xong. Mười phút phỏng vấn mua lại vài giờ sửa.

Hai lưu ý nhỏ cho vòng lặp tinh chỉnh. Output đầu tiên hiếm khi là bản cuối; cứ yêu cầu refactor, đơn giản hoá, đổi tên, đó là phần rẻ. Và nhớ training cutoff: thư viện có breaking change sau thời điểm model được train thì phải dán doc mới vào prompt, không thì nó dùng API cũ một cách rất tự tin — đây là một dạng bịa có lịch hẹn trước.
...
**Hiểu trước khi nói.** Đây là lần đầu bốn pattern được giới thiệu bằng tên — các slide trước mới cho khán giả chạm vào cơ chế (vòng ReAct, sub-agent, CLAUDE.md, plan mode) chứ chưa đặt tên kiến trúc. Vì vậy slide này phải giải thích thật, không phải ôn lại. Mỗi pattern trả lời một câu: nó tồn tại để giải vấn đề gì?

*Planner/Executor* giải vấn đề: giao một mục tiêu lớn cho model làm trong một mạch thì giữa chừng nó lạc đề. Tách hai vai: planner nhận mục tiêu, chia thành các task có thứ tự và phụ thuộc, ghi rõ mỗi task cần tool gì; executor nhận từng task phạm vi hẹp, trả kết quả có cấu trúc. Tách "quyết làm gì" khỏi "làm thế nào". *Multi-agent* giải vấn đề: context một agent hữu hạn và mục dần (slide 6). Thay một agent to ôm hết bằng nhiều agent chuyên biệt, mỗi agent một context riêng, có lớp điều phối; agent đọc hai mươi file chỉ trả về tóm tắt nên context chính không phải gánh. *Memory-augmented* giải vấn đề: model không nhớ gì giữa hai phiên. Ba lớp bộ nhớ: ngắn hạn là context phiên; dài hạn là file sống qua phiên (CLAUDE.md, todo.md); có cấu trúc là repo, lockfile, DB — nguồn sự thật có thẩm quyền, model không cần nhớ, chỉ cần tra. *Tool-using* giải vấn đề: model chỉ sinh được chữ, tool là tay chân để chạm vào thế giới — chính là nút Act của vòng ReAct. Bài gốc đặt bốn điều kiện: tool deterministic và định nghĩa rõ; quyền truy cập được kiểm soát; output có cấu trúc; thao tác nhạy cảm qua lớp duyệt. Mỗi pattern có một chỗ hỏng đặc trưng, và mỗi chỗ hỏng ứng với một bước SOP.
...
**Đọc.** Bốn card trên slide là bốn pattern kiến trúc mà mọi agentic coding tool hiện nay đều ghép từ đó mà thành, dù mỗi tool gọi bằng tên thương mại khác nhau. Suốt buổi sáng mọi người đã chạm vào cả bốn rồi mà chưa gọi tên, nên slide này tôi đi chậm: với mỗi pattern, bốn nhịp — nó tồn tại để giải vấn đề gì, bạn đã gặp nó ở đâu, nó hỏng chỗ nào, và bước SOP nào bịt chỗ hỏng đó.

Card một, Planner và Executor. Vấn đề nó giải: giao nguyên một mục tiêu lớn cho model làm trong một mạch, đến giữa chừng nó lạc đề — giống giao cả epic cho một dev mà không chia ticket. Pattern tách hai vai. Vai planner nhận mục tiêu, chia thành các task có thứ tự và phụ thuộc, ghi rõ mỗi task cần tool gì. Các vai executor nhận từng task với phạm vi hẹp, làm xong trả kết quả có cấu trúc cho planner. Bản chất là tách "quyết làm gì" khỏi "làm thế nào", như tổ chức tách chiến lược khỏi vận hành. Bạn đã gặp nó rồi: plan mode và bốn ga ở slide trước chính là pattern này được ép vào quy trình. Hỏng ở đâu? Plan sai thì mọi bước sau sai một cách rất nhất quán — executor làm đúng cái nó được giao, kể cả khi cái được giao là sai; và executor phạm vi rộng sẽ sửa cả thứ không được giao. Vá bằng: duyệt spec trước khi có dòng code nào, và mỗi task nhỏ kèm một test. Trụ Context.

Card hai, Multi-agent. Vấn đề nó giải: quay lại slide 6 — context của một agent là hữu hạn và mục dần khi đầy. Thay vì một agent to ôm hết mọi việc, chia thành nhiều agent chuyên biệt, mỗi agent một context riêng: một đứa chuyên đọc, một đứa chuyên tìm, một đứa chuyên review, và một lớp điều phối gom kết quả. Cái khôn của pattern nằm ở một chỗ: agent đi đọc hai mươi file chỉ trả về một đoạn tóm tắt, nên context của agent chính không bao giờ phải gánh hai mươi file. Bạn đã gặp nó rồi: kỹ thuật sub-agents ở slide 11 chính là nó. Hỏng ở đâu? Vai chồng lấn, không ai sở hữu kết quả cuối, và nguy hiểm nhất: agent này tin lời agent kia như tin sự thật. Có ai từng thấy hai agent tự khen nhau "looks good to me" rồi merge chưa? Vá bằng: tách Writer và Reviewer thành hai phiên có context khác nhau — cách làm cụ thể tôi đưa ngay ở phần review sau đây — sub-agent chỉ trả về tóm tắt, và đúng một người ký. Trụ Ownership.

Card ba, Memory-augmented. Vấn đề nó giải: model không nhớ gì giữa hai phiên — đóng tab là mất sạch, mai mở lại nó là người lạ. Pattern thêm ba lớp bộ nhớ, nhìn ba tầng trên card. Ngắn hạn: context của phiên hiện tại, trạng thái task đang làm. Dài hạn: file sống qua nhiều phiên — CLAUDE.md, todo.md — model đọc lúc bắt đầu, cập nhật lúc làm. Có cấu trúc: repo, lockfile, DB — đây là tầng hay bị quên: nguồn sự thật có thẩm quyền, model không cần nhớ, chỉ cần tra. Bạn đã gặp nó rồi: note-taking và file quy ước ở slide 11 chính là tầng giữa. Hỏng ở đâu? Bộ nhớ cũ hoặc sai làm agent lặp lại đúng một lỗi ở mọi phiên, và sau một tháng cả team tưởng đó là quy ước. Ví dụ rất đời: một dòng CLAUDE.md ghi "dùng yarn" từ trước khi repo chuyển sang pnpm — agent fail install ở mọi phiên, phiên nào cũng mất mấy vòng loay hoay, và không ai nghĩ đến việc mở file quy ước ra xem. Bài gốc có một câu đáng giữ: agent nhớ được thì phải chịu trách nhiệm được — bộ nhớ cũng cần người quản. Vá bằng: giữ CLAUDE.md ngắn và đúng, sửa ngay khi nó sai, clear khi context bẩn. Trụ Context.

Card bốn, Tool-using. Vấn đề nó giải: model chỉ sinh được chữ. Tool là tay chân để nó chạm vào thế giới thật — đây chính là nút Act trong vòng ReAct ở slide 7, giờ nhìn từ góc kiến trúc. Bài gốc đặt bốn điều kiện cho bộ tay chân đó: tool phải deterministic và được định nghĩa rõ; quyền truy cập được kiểm soát; output có cấu trúc để suy luận tiếp được; và thao tác nhạy cảm phải qua một lớp duyệt. Bạn đã gặp nó rồi: cái permission prompt hỏi bạn trước khi chạy lệnh chính là điều kiện thứ tư đang chạy trước mắt. Hỏng ở đâu? Quyền ghi sai và package sai — nhìn cái ổ khoá đỏ ở tool thứ tư trên card. Hai sự cố thật về đúng chuyện này sắp đến: một ở hai slide nữa, một ở phần của Nhân. Vá bằng: quyền tối thiểu cho từng tool; đọc và chạy test thì tự do, cài package, push, chạm production thì qua người. Trụ Verification và Ownership.
...
Lưới bốn ô là Diátaxis, một khung phân loại docs mà tôi ước mình biết sớm hơn. Hai trục: người đọc đang học hay đang làm, và họ cần thực hành hay lý thuyết. Ra bốn loại. Tutorial để học, cầm tay chỉ việc, một đường thẳng từ zero đến kết quả đầu tiên. How-to để làm một việc cụ thể, có mục tiêu rõ, cho người đã biết cơ bản. Reference để tra cứu chính xác, khô, đầy đủ. Explanation để hiểu vì sao. Cách nhanh nhất để khung này "click" là lấy một chủ đề duy nhất viết theo bốn kiểu. Cùng là pagination: tutorial là "xây endpoint có phân trang đầu tiên của bạn"; how-to là "cách thêm phân trang vào một endpoint có sẵn"; reference là bảng tham số page, limit và mã lỗi; explanation là "vì sao chọn cursor thay vì offset". Cùng một chủ đề, bốn tài liệu khác nhau, cho bốn người đọc khác nhau. AI viết loại nào cũng nhanh. Nhưng chọn loại nào cho ai là việc của bạn. Có ai từng mở một trang docs mà nửa đầu là tutorial, nửa sau là reference, xen vài đoạn triết lý, và không tìm được thứ mình cần? Đó là vì trộn bốn loại vào một trang.
